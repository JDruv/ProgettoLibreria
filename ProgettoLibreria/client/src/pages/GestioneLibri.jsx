...
function GestioneLibri() {
  ...
  useEffect(() => {
    const caricaDati = async () => {
      ...
      await Promise.all([
        ...
        (async () => {
          try {
            const response = await fetch('http://localhost:5000/api/prestiti', { headers: getAuthHeaders() });
  ...
  const handleConfirmDelete = async () => {
    ...
    const isBookOnLoan = prestiti.some(prestito => prestito.libro && prestito.libro._id === bookToDelete && !prestito.restituito);

    if (isBookOnLoan) {
      showNotification("Questo libro è attualmente in prestito e non può essere eliminato.", 'error');
